...
################################################################################
#                                                                              #
# python2 script to identify t3ss effectors using mixed hmm and blast seearches#
#                                                                              #
...
vfdb_folder = 'vfdb'
control_desc = open('%s/VFDB_T3SS_selected_Protein.headers' %vfdb_folder).read()
control_ids = {}
...
vfdb_prot_len = {}
for block in open('vfdb/VFDB_T3SS_selected_Protein.fas').read().split('>'):
    if block == '':
        continue
    block = block.strip()
...
def run_blastp(group):
    ...
    db1 = 'vfdb/VFDB_T3SS_selected_Protein.fas' 
